...
behavior Monitor(i_img_receiver ImgIn, i_time_receiver StartTime)
{
	...
	******************************************************************************/
	...
	void main(void)
	{
	   ...
	   int i, n;
	   sim_time t1, t2, t;
	   sim_time t3,t4;	//record previous t2
	   ...
	   for(i=0; i<IMG_NUM; i++)
	   {
	      ...
	      t3 = t2;		//record the time of previous output
	      t2 = now();
	      t = t2 - t1;
	      t4 = t2 - t3;     //frame throughput
	      n = i % AVAIL_IMG;
	      printf("%9s: Monitor received frame%3d with%9s "TIME_BASE_UNIT" delay.\n",
			time2str(buf2, t2/TIME_BASE), n+1, time2str(buf1, t/TIME_BASE));
	      printf("%9s:%4.3f seconds after previous frame,%2.3f FPS\n", time2str(buf2,t2/TIME_BASE),(float)(t4/(float)TIME_BASE2), 1/(float)(t4/(float)TIME_BASE2) );
		...
		in int rowStart, in int rowEnd, out float tempim[SIZE])
{
	...
	void main(void)
	{
	   ...
	   //waitfor ((821.7 MICRO_SEC)/20)/8;
	   waitfor (821.7 MILLI_SEC)/8;
	   blur_x(ROWS, COLS);
	}
};
		...
		in int colStart, in int colEnd, out simg smoothedim)
{
	...
	void main(void)
	{
	   ...
	   //waitfor ((895.5 MICRO_SEC)/20)/8;
	   waitfor (895.5 MILLI_SEC)/8;
	   blur_y(ROWS, COLS);
	}
};
